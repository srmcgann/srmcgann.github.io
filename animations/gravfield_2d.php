...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      g=x.createLinearGradient(0,0,0,w=c.width)
      g.addColorStop(0,'#7af5')
      g.addColorStop(1,'#ffd5')
      x.fillStyle=g,x.fillRect(0,0,w,w),h=800
      
      cl=150
      rw=cl*.5625|0
      iS=w/cl*4,eX=18000,dmp=1.1
      elasticity=20
      
      Q=(X,Y,s,cl)=>x.fillRect(X-s/2,Y-s/2,s,s)
      Rn=Math.random
      
      if(!t){
        B=[]
        for(P=[],i=cl*rw;i--;)P=[...P,[a=(i%cl)*w/cl+(w/cl/2),b=(i/cl|0)*1080/rw+(1080/rw/2),0,0,a,b]]
      }
      ...
      P.map(v=>{
        B.map(q=>{
          d=eX/(10+Math.hypot(a=v[0]-q[0],b=v[1]-q[1])**1.35)*q[2]
          v[2]+=S(p=Math.atan2(a,b))*d
          v[3]+=C(p)*d
        })
        ...
        X=v[0]+=v[2]/=dmp
        Y=v[1]+=v[3]/=dmp
        x.fillStyle=`hsla(${d=Math.hypot(v[2],v[3])*8},99%,${10+d}%,.05`
        Q(X,Y,iS)
      })
