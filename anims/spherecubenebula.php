...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.globalAlpha=.2,w=1920,h=540
      x.drawImage(bg,0,0,w,1080)
      x.globalAlpha=1
      x.fillStyle='#0004'
      ...
      for(i=rw**3;i--;){
        ...
        Y=((i/rw%rw)|0)-l
        Z=(i/(rw**2)|0)-l
        s=1+(-1+S(t))/(1+(H=(M=Math).hypot)(X,Y,Z)**.5)*3
        X=S(p=(A=M.atan2)(X,Z)+t*2)*(d=H(X,Z))*s
        Y*=s
        Z=C(p)*d*s+6
        if(Z>.5)with(x)beginPath(),arc(960+X/Z*h,h+Y/Z*h,Math.min(300,200/Z/Z),0,7),fillStyle=`hsla(${200+-t*800},80%,85%,.2`,fill()
      }
