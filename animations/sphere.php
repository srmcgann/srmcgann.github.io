...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      rws = 10,h=540
      x.lineJoin = 'round'
      Q=(a,b)=>x.lineTo(960+(S(p=Math.PI*2/cols*a+t/2)*S(q=Math.PI/rws*b)*r)/(Z=2.65+C(p)*S(q)*r)*h,h+(.25+C(q)*r)/Z*h-60,x.lineWidth=100/Z**4)
      x.strokeStyle='#1ca6'
      for(i=cols;i--;){
        for(j=rws;j--;){
          x[b='beginPath'](),q=Q(i,j),q=Q(i+1,j),x.stroke()
          x[b],q=Q(i,j),q=Q(i,j+1),x.stroke()
        }
      }
