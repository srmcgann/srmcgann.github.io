(function(){ 
    ...
    function draw(vars){
		...
		for(var i=0;i<vars.balls2.length;++i){
			...
			if(point.d != -1){
				size=800*vars.balls2[i].radius/(1+point.d);
				vars.ctx.drawImage(vars.balls2[i].index==vars.selected?vars.ball3Pic:vars.ball2Pic,point.x-size/2,point.y-size/2,size,size);
			}
		}
	}
    ...
    function loadScene(vars){
		...
		for(var i=0;i<sd;++i){
			...
			y=0;
			z=Math.cos(p)*16;
			vars.balls.push(new Vert(x,y,z,10));
		}
    }
