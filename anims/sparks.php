...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#0003',x.fillRect(0,0,w=c.width,w),h=800
      ...
      if(!t){
        s=
        P=[[0,0,0,0,iPv,0,0,0,0,iPs]]
      }

      if(!((t*60|0)%1)){
        for(l=P.length;l--;){
          if(Rn()<.2&&P.length<iMPl)for(m=6;m--;)P=[...P,[P[l][0],P[l][1],P[l][2],P[l][3]+(Rn()-.5)/100,P[l][4]+(Rn()-.5)/100,P[l][5]+(Rn()-.5)/100,0,0,0,iPs*Rn()]]
        }
      }
      ...
      P.map((v,i)=>{
        ...
        v[3]=v[3]/d*iPv/iPs*v[9]
        v[4]=v[4]/d*iPv/iPs*v[9]
        v[5]=v[5]/d*iPv/iPs*v[9]
        if(X>iBs*.75)v[0]-=iBs*1.5
        if(X<-iBs*.75)v[0]+=iBs*1.5
        if(Y>iBs/2)v[1]-=iBs
        if(Y<-iBs/2)v[1]+=iBs
        if(Z>iBs/2)v[2]-=iBs
        ...
        if(i&&Z>1){
          x.beginPath()
          s=(v[9]/=i?1.04:1)*10/Z/Z
          x.arc(...Q(),s,0,7)
          x.fillStyle=`hsla(${(d=Math.hypot(v[3],v[4],v[5]))*550},99%,${25+d*550}%,${.05+d*4}`
          x.fill()
        }
        return v
      })
      P=P.filter((v,i)=>(!i)||v[9]>5)
