...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.globalAlpha = .1
      x.drawImage(bg,0,0,w=c.width,c.height)
      x.globalAlpha=1
      x.fillStyle='#0001',x.fillRect(0,0,w,w),h=800
      P=t?P:[]
      
      oX=0,oY=0,oZ=10
      Rl=S(t/2)/4,Pt=C(t/2)/2,Yw=0
      iR=9,iS=30
      ...
      for(m=50;m--;)P=[...P,[S(p=Math.PI*2*Rn())*(d=(.5+iR*(Rn()))**1.35),-1.5,C(p)*d]]

      P=P.map(v=>{
        X=v[0],Y=v[1],Z=v[2]
        d=Math.max(0,Math.hypot(X,Z)-.03)
        X=S(p=Math.atan2(X,Z)+.05/d+.002)*d
        Z=C(p)*d
        Y+=.2/(.9+d**3)
        v[0]=X,v[1]=Y,v[2]=Z
        R()
        ...
        s=iS*40/Z/Z
        if(Z>1){
          x.fillStyle=`hsla(${d*30+t*250},99%,${100-d*4}%,${(7-Y)**2/800}`
          l=Q()
