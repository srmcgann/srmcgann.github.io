...
o=160,rw=90,sz=12;O=q=>q.map((v,i)=>{T(i);if(v[0])x[f]=v[0]-1?'#eff':`hsl(${t*300} 40%20%`,x[l](X*sz,Y*sz,sz,sz)});R=q=>{dn=0,P=Array(o*rw).fill().map((v,i)=>{T(i);return[(!X||!Y||X==o-1||Y==rw-1)?1:0,0]})};W=q=>{X=q%o,Y=q/o|0;return Y&&Y<rw&&X>0&&X<o-1&&!P[q][0]};D=(q,p)=>{h=P[q-o-1][0],b=P[q+o-1][0],d=P[q+o+1][0],e=P[q-o+1][0],g=p?p==1?d+e:p==2?h+e:b+d:h+b;return P[q-1][0]+P[q+1][0]+P[q-o][0]+P[q+o][0]+g==1};Q=(a,b)=>a+b*o;if(!t){T=q=>[X=q%o,Y=q/o|0],R(),dn=0;dn&&R();while(!dn){dn=1,a=P.map((v,i)=>v[0]&&v[1]<4?i:0).filter(v=>v);for(k=4e3;k--;){i=a[(r=Math.random)()*(a.length-1)|0],n=(p=r()*4|0)?p==1?i+1:p==2?i-o:i+o:i-1;if(W(n)){P[n][1]++;if(D(n,p)){P[n][0]=1,dn=0}}}}};x[l='fillRect'](0,0,w=c.width|=0,w),x[f='fillStyle']='#095',O(P);B=v=>(r()-.5)*80;t||(bxv1=B()-20,byv1=B()+20,bxv2=B(),byv2=B(),bx=[],by=[]);bx[0]=t?bx[0]+=(bxv1*=bx[0]>w-sz*2||bx[0]<sz*2?-1:1):w/2;by[0]=t?by[0]+=(byv1*=by[0]>1080-sz*4||by[0]<sz*4?-1:1):540;bx[1]=t?bx[1]+=(bxv2*=bx[1]>w-sz*2||bx[1]<sz*2?-1:1):w/2;by[1]=t?by[1]+=(byv2*=by[1]>1080-sz*4||by[1]<sz*4?-1:1):540;R=q=>{if(G[q]==-1||P[q][0])return false;G[q]=-1;if(q==s[1]||R(q+1)||R(q-1)||R(q-o)||R(q+o))return G[q]=1};for(s=[],i=2;i--;x.clearRect(bx[i]-20,by[i]-20,40,40))s[i]=Q(bx[i]/sz|0,by[i]/sz|0);s=s.map(v=>{j=0,d=v;while(j++<16&&P[d][0])d=v+(r()<.5?-1:1)+o*(r()<.5?-1:1);return d});G=Array(o*rw).fill(0),R(s[0]),O(G.map(v=>[v?v+1:0]))
