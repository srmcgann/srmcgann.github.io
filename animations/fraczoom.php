    ...
    Draw = () => {
      ...
      if(!t){
        Rs=(X,Y,s,sd,os)=>{
          ...
          let a=[]
          for(i=sd;i--;){
            sX=X+S(p=Math.PI*2/sd*i)*s
            sY=Y+C(p)*s
            if(sY<mind-s/2)mind=sY-s/2
            a=[...a,[sX,sY]]
            P=[...P,[sX,sY,s,os]]
          }
          a.map(v=>{
      ...
      P.map((v,i)=>{
        ...
        X=v[0]
        Y=v[1]-mind-400
        p=Math.atan2(X,Y)+t/2
        X=960+S(p)*(d=Math.hypot(X,Y))
        Y=540+C(p)*d
        x.lineTo(X,Y)
        if(i%sd==sd-1){
          x.closePath()
          x.strokeStyle=`hsla(${180+t*99},70%,60%,${Math.min(.3,(200+v[2]**3)/200000/(1+v[2]**5/3000000000))}`
          x.lineWidth=1+v[2]/5
          x.stroke()
          x.strokeStyle=`hsla(${180+t*99},99%,90%,${Math.min(.7,(200+v[2]**3)/200000/(1+v[2]**5/3000000000))}`
          x.lineWidth=1+v[2]/20
          x.stroke()
        }
      })
