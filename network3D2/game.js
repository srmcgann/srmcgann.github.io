(function(){ 
    ...
    function process(vars){
		...
		t = Math.sin(vars.frameNo / 160) / 120;
		...
		for(var i=0;i<vars.cells.length;++i){
			...
			for(var j=0;j<vars.cells.length;++j){
				if(j!=i){
					...
					if(d<vars.boundingCube*vars.boundingCube/50){
		...
		for(var i=0;i<vars.cells.length;++i){
			for (var key in vars.cells[i].segs) {
				...
				if(point1.d!=-1){
					...
					if(point2.d!=-1){
						a=1+vars.cells[i].segs[key].dist/20;
						vars.cells[i].color=-Math.PI/2+((vars.cells[i].rx-vars.cells[i].x)*(vars.cells[i].rx-vars.cells[i].x)+
														(vars.cells[i].ry-vars.cells[i].y)*(vars.cells[i].ry-vars.cells[i].y))/80;
			...
			if(point.d != -1){
				vars.ctx.fillStyle="#80f";
				size=1500000/(1+point.d*point.d);
    ...
    function loadScene(vars){
		...
		var initCells=800, initV=.75, x, y, vx, vy, size=10;
		vars.cells=[];
		vars.droplets=[];
