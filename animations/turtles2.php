...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.globalAlpha = .15
			s=1.2
			x.drawImage(bg, c.width/2-bg.width/4*s,c.height/2-bg.height/4*s,bg.width/2*s,bg.height/2*s)
			x.globalAlpha = 1
			x.fillStyle='#0004',x.fillRect(0,0,w=c.width,w)
      
      iTs=80,iTv=2.5,iTr=8,iTt=100,iTa=1
      ...
      Q=q=>{
        for(m=q[5]/12|0;m--;){
          P=[...P,[q[0],q[1],10+(q[2]/2-10)*Rn(),q[3],0,100]]
        }
      }
      ...
      P.map((v, j)=>{
        ...
        x.lineWidth=v[2]/8
        x.fillStyle=x.strokeStyle=`hsla(${t*50+290-v[5]*.5},99%,${75+S(t*2+v[3])*25}%,.4`
        x.stroke()
        x.fill()
        if(v[2]>=iTt){
          Q(v)
