...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#000',x.fillRect(0,0,w=c.width,w),h=800
      ...
      Rl=0,Pt=Math.PI/2-t/3,Yw=Math.PI/2+t
      cl=80,rw=cl*.5,oX=0,oY=0,oZ=7,iR=3

      N=q=>{
        X=S(p=Math.PI*2/cl*q)*S(q=Math.PI/rw*(q/cl|0))*iR
        Y=C(q)*iR
        Z=C(p)*S(q)*iR
        X*=1+S(l=S(p*4+t*2)*S(q*4+t*2))/4*(v=S(t*2))
        Y*=1+S(l)/4*v
        Z*=1+S(l)/4*v
        R()
        x.lineWidth=50/(Z**1.25)
        return Q()
      }
      x.lineCap='round'
      x.strokeStyle='#afc3'
