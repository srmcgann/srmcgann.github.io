...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.globalAlpha=.4,x.drawImage(bg, 0,0,w=c.width,w),h=540
      x.globalAlpha=1
      turns=10
      iterations=99
      radius=6
      ...
      R=(X,Y,Z)=>{X=S(p=(A=(M=Math).atan2)(X,Y)+Rl)*(d=(H=M.hypot)(X,Y)),Y=C(p)*d,Y=S(p=A(Y,Z)+Pt)*(d=H(Y,Z)),Z=C(p)*d,X=S(p=A(X,Z)+Yw)*(d=H(X,Z)),Z=C(p)*d;return [X,Y,Z]}
      Q=q=>[960+X/Z*h,h+Y/Z*h]
      Array(iterations).fill().map((v,i)=>[X=S(p=Math.PI/iterations*turns*i+t*2),Y=C(p),Z=0,-X,-Y,0]).map((v,i)=>{Yw=Math.PI*2/iterations*i;return [...R(v[0]+radius,v[1],v[2]),...R(v[3]+radius,v[4],v[5])]}).map((v,i)=>{
        x.beginPath()
        Pt=Math.PI/4+t/4
        Yw=t/1.5
        Rl=0
        ...
        f=Z=b[2]+oZ
        x.lineTo(...(b=Q()))
        x.strokeStyle=`hsla(${360/iterations*i},99%,${75+S(Math.PI*2/iterations*i-t*8)*50}%,.35)`
        x.lineWidth=100/Z
        x.stroke()
