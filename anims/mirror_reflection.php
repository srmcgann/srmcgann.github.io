...
x.fillStyle='#0004',x.fillRect(0,0,w=c.width,w),h=800;R=()=>{X=S(p=A(X,Y)+Rl)*(d=(H=(M.hypot))(X,Y)),Y=C(p)*d,Y=S(p=A(Y,Z)+Pt)*(d=H(Y,Z)),Z=C(p)*d,X=S(p=A(X,Z)+Yw)*(d=H(X,Z)),Z=C(p)*d,Z+=oZ};Rn=Math.random;Q=q=>[960+X/Z*h,540+Y/Z*h],G=5,oZ=G*4.25,Rl=0,Pt=0,Yw=0,iPv=.5,iPs=18,iPrr=.4,iPc=6,sql=.2;if(!t)x.lineCap='round',P=Array(iPc).fill().map(v=>[-G/2,0,0,S(p=Rn()*7)*iPv,0,C(p)*iPv,[]]);P.map(v=>{p=(A=(M=Math).atan2)(v[3],v[5])+(Rn()-.5)*sql,v[1]=3+S(t)*3,v[3]=S(p)*iPv,v[5]=C(p)*iPv,v[0]+=v[3],v[1]+=v[4],v[2]+=v[5];if(v[0]>0||v[0]<-G*2)v[3]*=-1;if(v[2]>G*2||v[2]<-G*2)v[5]*=-1;v[6].push([v[0],v[1],v[2],iPs]);v[6].map((q,i)=>{q[3]-=iPrr;q[2]-=.4;if(i){for(m=4;m--;){X=q[0]*(m%2?-1:1),Y=q[1]*(m<2?-1:1),Z=q[2],R(),x.beginPath(),x.lineTo(...Q()),X=v[6][i-1][0]*(m%2?-1:1),Y=v[6][i-1][1]*(m<2?-1:1),Z=v[6][i-1][2],R();if(Z>.5)with(x)lineTo(...Q()),lineWidth=q[3]*500/((1+Z)**2),strokeStyle=`hsla(${i*40+t*400},99%,${100-(50/((1+1/v[6].length*i)**4))}%,.1`,stroke()}}});v[6]=v[6].filter(v=>v[3]>.1)})
