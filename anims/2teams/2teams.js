...
function loadScore(){
	...
	loader.load( 'Square721 BT_Roman.json', function ( font) {
		...
		textMaterial = new THREE.MeshPhongMaterial( { color: 0x4444ff, specular: 0xffffff } );
		mesh = new THREE.Mesh( textGeometry, textMaterial );
		mesh.material.opacity = 1;
		mesh.position.x=-20;
		mesh.position.y=120;
	...
	loader.load( 'Square721 BT_Roman.json', function ( font) {
		...
		textMaterial = new THREE.MeshPhongMaterial( { color: 0xff4444, specular: 0xffffff } );
		mesh = new THREE.Mesh( textGeometry, textMaterial );
		mesh.material.opacity = 1;
		mesh.position.x=-19;
		mesh.position.y=105;
...
function loadScene(){
	...
	plane = new THREE.Mesh( new THREE.PlaneGeometry( 200000, 100000 ), new THREE.MeshBasicMaterial( { transparent: true, opacity: 0.275 } ) );
...
function victory(id){
	...
	renderer.setClearColor( interpolateColors(id?[1,0,0]:[0,0,1],[0,0,0],Math.cos(frames/20)/2));
	plane.material.color.setHex(interpolateColors(id?[1,0,0]:[0,0,1],[0,0,0],Math.cos(frames/20)/2));
	if(!textLoaded){
		textLoaded=1;
		purpleScore+=id?0:1;
		greenScore+=id?1:0;
    ...
    startRecording = () => {
      ...
      const chunks = []
      const stream = document.querySelectorAll('canvas')[0].captureStream()
