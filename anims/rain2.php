...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      Rn=Math.random
      //g=x.createLinearGradient(0,0,0,w=c.width),g[a='addColorStop'](0,'#8af3'),g[a](1,'#ffe3'),x[f='fillStyle']=g
      I=(A,B,M,D,E,F,G,H)=>(K=((G-E)*(B-F)-(H-F)*(A-E))/(J=(H-F)*(M-A)-(G-E)*(D-B)))>=0&&K<=1&&(L=((M-A)*(B-F)-(D-B)*(A-E))/J)>=0&&L<=1?[A+K*(M-A),B+K*(D-B)]:0;
      x.fillStyle='#0003'
      x.fillRect(0,0,w=c.width,w),h=800

      iV=12,iPs=30,bn=iV*2
      ...
      for(m=12;m--;)P=[...P,[0,0,S(p=Math.PI/2-Rn())*iV,C(p)*iV,iPs],[w,0,-S(p=Math.PI/2-Rn())*iV,C(p)*iV,iPs]]

      x.beginPath()
      x.lineCap='round'
      x.lineJoin='round'
      for(i=26;i--;){
        X=960+S(p=Math.PI/24*i-Math.PI/2)*400
        Y=700+C(p)*(C(p))*500*S(t)/2-S(t)*70
        if(i<25)x.lineTo(X,Y)
        P.map(v=>{
          tVx1=v[0]+v[2]*1
          tVy1=v[1]+v[3]*1
          tVx2=v[0]+v[2]*4
          tVy2=v[1]+v[3]*4
          if(i<24&&(a=I(X,Y,oX,oY,tVx1,tVy1,tVx2,tVy2))){
            v[2]+=S(p=Math.atan2(X-oX,Y-oY)-Math.PI/2)*bn
            v[3]+=C(p)*bn
          }
          return v
        })
      ...
      P=P.map(v=>{
        ...
        x.beginPath()
        x.fillStyle='#6af1'
        x.arc(v[0],v[1],v[4]/=1.005,0,7)
        x.fill()
        return v
      }).filter(v=>v[1]<1080+iPs&&v[4]>5)
