...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#0008',x.fillRect(0,0,w=c.width,w),h=540,oPX=oPY=oPZ=0
      ...
      sd=6,tw=0,oR=3.25,yaw=0,iR=.4,cl=60,oX=0,oY=0,oZ=9
      l=sd+3
      mR=0,mP=Math.PI/2,mY=t/2,iT=t
      ...
      for(j=cl*4+1;j--;){
        ...
        x.strokeStyle=`hsla(${360/cl/4*j},50%,${85+S(Math.PI*2/cl/4*j+t*10)*25}%,.6)`
        for(i=sd;j<cl*4&&i--;){
          x.beginPath()
          for(k=2;k--;){
            ...
            Y=C(p)*iR
            Z=0
            vX=S(q=Math.PI/2/cl*v)*2
            vY=S(p=Math.PI*3/cl*v)*3
            vZ=C(q)*2
            nX=S(q=Math.PI/2/cl*(v-l))*2
            nY=S(p=Math.PI*3/cl*(v-l))*3
            nZ=C(q)*2
            R(0,E(nX,nY,nZ),Math.atan2(k?PX:oPX,k?PZ:oPZ)+Math.PI/2)
            X+=(k?PX:oPX)+vX
            Y+=(k?PY:oPY)+vY
            Z+=(k?PZ:oPZ)+vZ
            R(mR,mP,mY)
        ...
        for(i=sd;j&&i--;){
          ...
          nY=S(p=Math.PI*3/cl*(v-l))*3
          nZ=C(q)*2
          R(0,E(nX,nY,nZ),Math.atan2(PX,PZ)+Math.PI/2)
          X+=PX+vX
          Y+=PY+vY
          Z+=PZ+vZ
          R(mR,mP,mY)
