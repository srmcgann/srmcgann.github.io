...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#fff4',x.fillRect(0,0,w=c.width,w)
      x.font='70px arial'
      for(j=20;j--;){
        d=((t*25+j)/20*500%w)
        x.lineWidth=25
        x.strokeStyle=`hsla(${d/4-t*500},50%,${100-2e4/d}%,${(1+S(Math.PI*2/20*j-Math.PI/2))/12*(d/199)}`
        x.beginPath()
        x.arc(960,540,d,0,7)
        x.stroke()
      }
      ...
      for(i=2400;i--;){
        ...
        X=(i%s) * v - v*2 - 60
        Y=(i/s|0) * v - v*6
        X=960+S(p=Math.atan2(e=X-960,b=Y-600)+t/4)*(d=Math.hypot(e,b)*g)
        Y=600+C(p)*d
        m=x.measureText(a[j]).width
        if(X>-m*2&&X<w+m*2&&Y>-160&&Y<1080+160){
          x.fillStyle=`hsla(${d/2-t*500},40%,${130-5e4/d}%,.5`
          x.fillText(a[j],X-m/2,Y-75)
        }
      }
