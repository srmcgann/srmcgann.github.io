...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#0005',x.fillRect(0,0,w=c.width,w),h=540,oX=0,oY=8,oZ=10,sd=4,rw=10,cl=12,P=[]
      Q=(X,Y,Z)=>[960+X/Z*h,100+Y/Z*h]
      R=(Pitch,Yaw)=>{Y=S(p=(A=(M=Math).atan2)(Y,Z)+Pitch)*(d=(H=M.hypot)(Y,Z)),Z=C(p)*d,X=S(p=A(X,Z)+Yaw)*(d=H(X,Z)),Z=C(p)*d}
      for(i=rw*cl*sd;i--;){
        j=i%sd
        X=S(p=Math.PI*2/sd*i+Math.PI/sd)*(d=1)+(((i/sd|0)%rw)*2-rw)*2
        Y=0
        a=Z=C(p)*d+((i/sd/rw|0)*2-cl-(t*2%1)*2)*2
        R(0,S(t/2)/2)
        X+=oX
        Y+=oY
        Z+=oZ+cl
        P=[...P,[X,Y,Z,1+(((i+(t*2|0)*sd*rw)/sd|0)**3.1)%4,Math.min(.45,((a+7+cl)**5)/1e12*(((cl+8)*2-(a+7+cl))**8.2/2e5)*2)]]
      }
      x.lineJoin='round',x.lineCap='round'
      P.map((v,i)=>{
        if(!(i%sd))x.beginPath()
        x.lineTo(...Q(...v))
        if(v[2]>2&&(i%sd)==sd-1)x.closePath(),x.lineWidth=500/v[2]/v[2],x.strokeStyle=`hsla(${t*99},99%,${800/(1+v[2]**2/15)}%,${v[4]}`,x.stroke()
      })
      P.map((v,i)=>{
        if(!(i%sd))x.beginPath()
        x.lineTo(...Q(v[0],v[1]-v[3],v[2]))
        if(v[2]>2&&(i%sd)==sd-1)x.closePath(),x.lineWidth=500/v[2]/v[2],x.strokeStyle=`hsla(${t*99},99%,${800/(1+v[2]**2/15)}%,${v[4]}`,x.stroke()
      })
      P.map((v,i)=>{
        x.beginPath()
        x.lineTo(...Q(v[0],v[1],v[2]))
        x.lineTo(...Q(v[0],v[1]-v[3],v[2]))
        if(v[2]>2)x.lineWidth=500/v[2]/v[2],x.strokeStyle=`hsla(${t*99},99%,${800/(1+v[2]**2/15)}%,${v[4]}`,x.stroke()
      })
