...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#0106',x.fillRect(0,0,w=c.width,w),h=800

      Rn=Math.random
      Q=(X,Y,r,cl)=>{x.beginPath(),x.arc(X,Y,r,0,7),x.fillStyle=cl,x.fill()}
      ...
      if(!t){
        for(B=[],i=iBc;i--;)B=[...B,[Rn()*w,Rn()*1080,S(p=Rn()*Math.PI)*iV,C(p)*iV]]
        for(P=[],i=mCl*mRw;i--;){
          X=(i%mCl)*(l=w/mCl)+l/2
          Y=(i/mCl|0)*(l=1080/mRw)+l/2
          for(j=sCl*sRw;j--;){
            P=[...P,[a=X+(j%sCl)*sw/sCl+sw/sCl/2-sw/2,b=Y+(j/sCl|0)*sh/sRw+sh/sRw/2-sh/2,0,0,a,b]]
          }
        }
      }
      B.map(v=>{
        ...
        v[0]+=v[2]
        v[1]+=v[3]
        if(v[0]>w-iBs||v[0]<iBs)v[2]*=-1
        if(v[1]>1080-iBs||v[1]<iBs)v[3]*=-1
        if(v[0]>w-iBs)v[0]=w-iBs
        if(v[0]<iBs)v[0]=iBs
        if(v[1]>1080-iBs)v[1]=1080-iBs
        if(v[1]<iBs)v[1]=iBs
        Q(v[0],v[1],iBs,'#26f')
      })
      P.map(v=>{
        B.map((q,i)=>{
          B.map((j,k)=>{
            if(k!=i){
              d=Math.hypot(a=j[0]-q[0],b=j[1]-q[1])
              if(d<(l=iBs*2)){
                p=Math.atan2(a,b)
                l+=1
                j[0]=q[0]+S(p)*l
        ...
        v[2]+=(v[4]-v[0])/elasticity
        v[3]+=(v[5]-v[1])/elasticity
        v[0]+=v[2]/=1.06
        v[1]+=v[3]/=1.06
        Q(v[0],v[1],sS,'#0f68')
      })
