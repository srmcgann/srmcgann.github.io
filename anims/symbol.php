  ...
  <script>
    ...
    Draw = () => {
      ...
      if(!t){
			  ...
			  R=(Rl,Pt,Yw,m)=>{X=S(p=(A=(M=Math).atan2)(X,Y)+Rl)*(d=(H=M.hypot)(X,Y)),Y=C(p)*d,Y=S(p=A(Y,Z)+Pt)*(d=H(Y,Z)),Z=C(p)*d,X=S(p=A(X,Z)+Yw)*(d=H(X,Z)),Z=C(p)*Z;if(m)X+=oX,Y+=oY,Z=oZ}
			  sd=24
			}
      ...
      Cr(0,0,0,2,0)
			...
			for(j=2;j--;){
				for(i=6;i--;){
          X1=S(p=Math.PI*2/6*i)
					Y1=C(p)
					Z1=0
					Cr(X1*(j+1),Y1*(j+1),Z1,.5,1)
					if(!j) Cr(X1*1.5,Y1*1.5,Z1,1,0)
			  }
			}
