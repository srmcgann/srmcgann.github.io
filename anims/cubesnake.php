...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      x.fillStyle='#0005',x.fillRect(0,0,w=c.width,w),h=800
      ...
      RC=q=>{
        ...
        for(let j=6;j--;){
          x.beginPath()
          for(let i=4;i--;){
            let a=[S(p=Math.PI*2/4*i+Math.PI/4)*(d=2**.5/2),C(p)*d,.5]
            X=q[0]+a[j%3]*(l=j<3?-1:1),Y=q[1]+a[(j+1)%3]*l,Z=q[2]+a[(j+2)%3]*l
            R()
            x.lineTo(...Q())
          }
          x.closePath
      ...
      P.map(v=>{
        x.strokeStyle=`hsla(${v[7]+t*300},99%,${(500-v[7])/5}%,${.5/(1+v[7]/70)}`
        v[7]+=2
        RC(v)
        ...
        if(v[6]>0){
          ...
          while(dn==-1){
            switch(Rn()*6|0){
              case 0: if(v[8]!=1&&v[0]>-G) TX=-1, dn=0; break
              case 1: if(v[8]!=0&&v[0]<G) TX=1, dn=1; break
              case 2: if(v[8]!=3&&v[1]>-G) TY=-1, dn=2; break
              case 3: if(v[8]!=2&&v[1]<G) TY=1, dn=3; break
              case 4: if(v[8]!=5&&v[2]>-G) TZ=-1, dn=4; break
              case 5: if(v[8]!=4&&v[2]<G) TZ=1, dn=5; break
            }
          }
          P=[...P,[v[0],v[1],v[2],v[0]+TX,v[1]+TY,v[2]+TZ,0,0,dn]]
        }else if(v[6]!=-1){
          v[0]+=(l=v[3]-v[0])>0?sp:l<0?-sp:0
          v[1]+=(l=v[4]-v[1])>0?sp:l<0?-sp:0
          v[2]+=(l=v[5]-v[2])>0?sp:l<0?-sp:0
          v[0]=Math.round(v[0]*100)/100
          v[1]=Math.round(v[1]*100)/100
          v[2]=Math.round(v[2]*100)/100
          if(v[0]==v[3]&&v[1]==v[4]&&v[2]==v[5])v[6]=1
        }
      })
      P=P.filter(v=>v[7]<500)
