...
<html>
  ...
  <script>
    ...
    Draw = () => {
      ...
      cl=25,rw=25,sp=1,spd=20/1.5
      oX=0,oY=0,oZ=rw/2*sp+10
      Rl=t/6/1.5,Pt=t/4/1.5,Yw=t/1.5

      x.lineJoin='round'
      for(i=0;i<cl*rw+1;++i){
        if(!(i%cl))x.beginPath()
        X=(i%cl)*sp-(cl/2*sp)
        Z=(l=i/cl|0)*sp-(rw/2*sp)-(e=t*spd%1)
        Y=(S(Math.PI*4/cl*i)+S(+Math.PI*4/rw*(l+(t*spd|0)*5/(sp*5))))*S(t*2)*2
        R()
        if(Z>1)x.lineTo(...Q())
        if((i%cl)==cl-1){
          x.strokeStyle=`hsla(${180},99%,50%,.3`
          x.lineWidth=7
          x.stroke()
        }
      }
      for(i=0;i<cl*rw+1;++i){
        if(!(i%cl))x.beginPath()
        Z=(i%cl)*sp-(cl/2*sp)-(e=t*spd%1)
        X=(l=i/cl|0)*sp-(rw/2*sp)
        Y=(S(Math.PI*4/cl*l)+S(+Math.PI*4/rw*(i+(t*spd|0)*5/(sp*5))))*S(t*2)*2
        R()
        if(Z>1)x.lineTo(...Q())
        if((i%cl)==cl-1){
          x.strokeStyle=`hsla(${180},99%,50%,.3`
